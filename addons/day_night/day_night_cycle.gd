...
@export var day_lenth_in_seconds: float = 5 :
	get:
...
@export var day_config: DayConfig = null

static var instance = null


@onready var on_day_change: Subject = Subject.new()
@onready var on_year_change: Subject = Subject.new()
@onready var on_night_time_start: Subject = Subject.new()
@onready var on_day_time_start: Subject = Subject.new()
@onready var on_time_paused: BehaviorSubject = BehaviorSubject.new(false)
@onready var on_time_resumed: BehaviorSubject = BehaviorSubject.new(false)
...
#	
	...
	o.as_observable().subscribe(func(is_time: bool): print("Reminder that is is 13 hours at the time: " + get_time().get_time_as_string())).dispose_with(self)
	o2.as_observable().subscribe(func(is_time: bool): print("Reminder that is is 12 hours at the time: " + get_time().get_time_as_string())).dispose_with(self)
	o3.as_observable().subscribe(func(is_time: bool): print("Reminder that is is 14 hours at the time: " + get_time().get_time_as_string())).dispose_with(self)
...
func _process(delta: float):
	...
	if not _time_stopped:
		_percentage_through_day += (delta / day_lenth_in_seconds) * _time_multiplier;
	...
	if (_percentage_through_day < _sunset_start_percentage) && (_percentage_through_day > _sunrise_start_percentage):
		if not _is_day:
			...
			if should_trigger_events:
				on_day_time_start.on_next(GameTime.new(get_time().get_epoch()))
		else:
			var delete_me
			#it has been day
			# DO NOTHING FOR NOW
		...
		if _is_day:
			...
			_is_day = false
			if should_trigger_events:
				on_night_time_start.on_next(GameTime.new(get_time().get_epoch()))
		else:
			var delete_me
			# it has been night
			# DO NOTHING FOR NOW
...
func create_reminder(time: GameTime, repeating: bool = false) -> Observable:
	...
	if time.is_today(get_time()) and time.is_after(get_time()):
		if repeating:
		...
		_reminder_index = 0
		while _todays_reminders[_reminder_index].time.is_before(get_time()):
			_reminder_index += 1
...
func _send_potential_reminders() -> void:
	if (_todays_reminders.size() > 0) and (_reminder_index < _todays_reminders.size()):
		if _todays_reminders[_reminder_index].time.is_before(get_time()):
